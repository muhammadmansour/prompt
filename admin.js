...
async function loadDashboard() {
  ...
  try {
    ...
    renderDashCardGrid(promptCount);
    console.log('[admin.js] Dashboard rendered');
  } catch (e) {
    console.error('[admin.js] loadDashboard error:', e);
    try { renderDashCardGrid(0); } catch (e2) { console.error('renderDashCardGrid error:', e2); }
  }
}

function renderDashCardGrid(promptCount) {
  const el = document.getElementById('dash-card-grid');
  if (!el) { console.error('dash-card-grid element not found'); return; }

  const totalSessions = sessions.length;
  const totalCollections = collections.length;
  const totalFrameworks = frameworks.length;
  const totalFiles = collections.reduce((a, c) => a + (c.file_counts?.active || c.fileCount || 0), 0);
  const totalCS = (csSessions || []).length;
  ...
  const cards = [
    {
      page: 'audit-studio',
      highlighted: true,
      icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M12 5V7M12 17V19M7 12H5M19 12H17M8 8L6.5 6.5M17.5 17.5L16 16M8 16L6.5 17.5M17.5 6.5L16 8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><circle cx="12" cy="12" r="4" stroke="currentColor" stroke-width="1.8"/></svg>`,
      title: 'Audit Studio',
      desc: 'Build context and start new audit sessions',
      stat: totalSessions + ' sessions',
    },
    {
      page: 'file-collections',
      icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M3 7V18C3 18.6 3.4 19 4 19H20C20.6 19 21 18.6 21 18V10C21 9.4 20.6 9 20 9H12L10 7H4C3.4 7 3 7.4 3 8Z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
      title: 'File Collections',
      desc: 'Manage reference documents and file collections',
      stat: totalCollections + ' collections · ' + totalFiles + ' files',
    },
    {
      page: 'audit-sessions',
      icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M4 5H20L22 7V19C22 19.6 21.6 20 21 20H3C2.4 20 2 19.6 2 19V6C2 5.4 2.4 5 3 5Z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M8 12H16M8 16H12" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>`,
      title: 'Audit Sessions',
      desc: 'View and manage all audit sessions',
      stat: totalSessions + ' sessions',
    },
    {
      page: 'controls-studio',
      icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="8" stroke="currentColor" stroke-width="1.8"/><path d="M8 12L11 15L16 10" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
      title: 'Controls Studio',
      desc: 'AI-powered control suggestions for requirements',
      stat: totalCS + ' sessions',
    },
    {
      page: 'merge-optimizer',
      icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M17 3V21M7 7V13C7 16.3 9.7 19 13 19H17M7 7L4 10M7 7L10 10" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
      title: 'Merge Optimizer',
      desc: 'Reduce redundancy by merging overlapping controls',
      stat: totalCS + ' studio sessions',
    },
    {
      page: 'org-contexts',
      icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M3 19V6C3 5.4 3.4 5 4 5H20C20.6 5 21 5.4 21 6V19M7 10H17M7 14H13M12 5V3" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
      title: 'Org Contexts',
      desc: 'Manage organization profiles for AI context',
      stat: '',
    },
    {
      page: 'prompts',
      icon: `<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M4 5H20M4 9H14M4 13H18M4 17H10" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>`,
      title: 'Prompts',
      desc: 'Manage and edit prompt templates',
      stat: promptCount + ' prompts',
    },
  ];

  el.innerHTML = cards.map(c => `
    <div class="dash-nav-card${c.highlighted ? ' dash-nav-card-highlight' : ''}" onclick="navigateTo('${esc(c.page)}')">
      <div class="dash-nav-card-top">
        <div class="dash-nav-card-body">
          <h3 class="dash-nav-card-title">${esc(c.title)}</h3>
          <p class="dash-nav-card-desc">${esc(c.desc)}</p>
          ${c.stat ? `<span class="dash-nav-card-stat">${esc(c.stat)}</span>` : ''}
        </div>
        <div class="dash-nav-card-icon${c.highlighted ? ' dash-nav-card-icon-hl' : ''}">
          ${c.icon}
        </div>
      </div>
      <div class="dash-nav-card-open">
        Open <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M3 2L7 5L3 8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
      </div>
    </div>
  `).join('');
}
